function name = simulation3D_compressing_bar(varargin)
close all
clf
...
nFixed = length(indBottom) + length(indTop);
...
indBottomLogical = logical(zeros(3,N));
indBottomLogical(:,indBottom) = logical(1);
indBottomLogical = indBottomLogical(:);

indTopLogical = logical(zeros(3,N));
indTopLogical(:,indTop) = logical(1);
indTopLogical(:);
