...
maxA_list = [1 0.1 0.01]';

interpolation_step_list = [0.1];
...
for i = 1:length(maxA_list)
    ...
    for i_deform = 1:length(interpolation_step_list)
        ...
        if maxA == 1
            ...
            trimesh(elem0, nodeM0(:,1),nodeM0(:,2));
            ...
            Xind_apex2 = (abs(nodeM(:,1) - max(nodeM(:,1))) < 1e-6) & (abs(nodeM(:,2) - min(nodeM(:,2))) < 1e-6);
            ind_apex2 = reshape(transpose(repmat(Xind_apex2,1,2)),[],1); % logical index for total position vector
            Xind_apex3 = (abs(nodeM(:,1) - min(nodeM(:,1))) < 1e-6) & (abs(nodeM(:,2) - min(nodeM(:,2))) < 1e-6);
