function name = simulation3D_jump_SI_not_rescaled_scoarsebunny_hete(varargin)
close all
clf
...
%%
meshname = sprintf('mesh_data%cscoarsebunny.1',fs);
...
bottom_points = find(abs(nodeM(:,3)-nodeM(1,3)) < (0.2/100));
...
soft_points = find(abs(nodeM(:,3)-min(nodeM(:,3))) < (0.7/100));
