function test_ERAVF_sine_gordon
addpath('expmv')
for test_alphas = 2
    for test_ks = 2
        close all
        ...
        x0 = zeros(1,n);
        v0 = 8./ cosh(2*((-d+h) : h : d));
        % multiply by Alpha to get enough initial energy
        ...
        ERavf = ERAVF(f, u0, Jv, 1, 0, true);
        
        % function obj = ERAVF(Hf, IC, HJ, NonlinearSolve, Integration, usingKSM)
        % NonlinearSolve: indicating which method to use for the nonlinear solve
        % % 0: use matlab build-in fsolve
        % % 1: use fixed-point iteration
        % % 2: use fixed-point iteration with expmv from Al-Mohy and Higham
        ...
        for i = 1:N
            ...
            title(['ERavf' ' SG' ' alpha ' num2str(Alpha)])
            figure(hf2)
