classdef EAVF < FirstOrderIVP
    ...
    properties
        ...
        NIT; % number of fixed point iteration for each step
    ...
    methods
        ...
        function sol = step(obj,k)
            ...
            % checking the splitting forms
%             disp('splitting error (should be 0)')
%             disp(norm(obj.Hf(t,u_n) - obj.Q*(obj.L * u_n + obj.HdU(u_n))))
            ...
            if obj.isUsingKSM
                ...
                projected_expM = expm(k*Hm);
                E1 = zeros(size(projected_expM,2),1); E1(1) = 1;
                linear_part = norm(v0) * Vm * projected_expM * E1;
            else
            ...
            switch obj.NonlinearSolve
                ...
                case 1
                    ...
                    u_np1 = u_n; % for checking with fixed point iteration
                    u_np1_old = u_np1;
                    fixedPointIteration = @(u_np1)  linear_part + k*obj.nonlinearDiscreteGradient(u_n, u_np1);
                    u_np1 = fixedPointIteration(u_np1);
                    it = 1;
                    while (norm(u_np1 - u_np1_old) > 1e-9 && it < obj.MaxIT)
%                     while (norm(u_np1 - u_np1_old) > 1e-9)
                        u_np1_old = u_np1;
                        u_np1 = fixedPointIteration(u_np1);
                        it = it + 1;
                    end
            ...
            obj.state(obj.CurrentIndex + 1,:) = u_np1';
