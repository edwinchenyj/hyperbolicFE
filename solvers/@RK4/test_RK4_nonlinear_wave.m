function test_RK4_nonlinear_wave
...
Jv = @(t, u) A - Alpha*spdiags(3*(u(1:n).^2),-n,sparse(2*n,2*n));
        ...
        for i = 1:N
            ...
            RK4_NLW.step(k);
            RK4_state = RK4_NLW.GetCurrentState;
            RK4_position = RK4_state(1:n);
            figure(hf1)
            
            plot((-d+h) : h : d, RK4_position)
            title(['RK4' ' NLW' ' alpha ' num2str(Alpha)])
            figure(hf2)
            
            RK4_H(i+1) = energy(RK4_state,h,Alpha);
            scatter(Time(1:i+1),RK4_H(1:i+1),'.');
            title(['RK4' ' NLW' ' alpha ' num2str(Alpha)])
            xlim([t0,N*k])
