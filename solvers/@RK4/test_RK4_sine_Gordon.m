...
for test_alphas = 2
    for test_ks = 3
        ...
        for i = 1:N
            ...
            RK4_SG.step(k);
            RK4_SG_state = RK4_SG.GetCurrentState;
            RK4_SG_position = RK4_SG_state(1:n);
            figure(hf1)
            
            plot((-d+h) : h : d, RK4_SG_position)
            title(['RK4' ' SG' ' alpha ' num2str(Alpha)])
            figure(hf2)
            
            RK4_SG_H(i+1) = energy(RK4_SG_state,h,Alpha);
            scatter(Time(1:i+1),RK4_SG_H(1:i+1),'.');
            title(['RK4' ' SG' ' alpha ' num2str(Alpha)])
            xlim([t0,N*k])
