function load_compare_simulation_polyfit
...
for i_maxA = 1:length(maxA_list)
    ...
    if polyfit_on
        simdir = strcat(dirname,fs,solver,'_',simulation_type,'_polyfit',num2str(NE),'_Y',num2str(Y),'_P',num2str(P),'_dt',num2str(dt));
    else
        simdir = strcat(dirname,fs,solver,'_',simulation_type,'_polyfit_off_Y',num2str(Y),'_P',num2str(P),'_dt',num2str(dt));
    end
