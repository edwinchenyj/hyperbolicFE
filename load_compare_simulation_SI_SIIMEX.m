function load_compare_simulation_SI_SIIMEX

clear all
close all

video_name = 'DGBZ_SI_SIMEX_1e-1.avi';
...
solver_list = {'SI','SIIMEX'};
filename_list = cell(1,length(solver_list));
...
for i_solver = 1:length(solver_list)
    ...
    filename_list{i_solver} = [simdir fs 'trajectory.mat'];
end
edge_colors = num2cell(hsv(length(solver_list)),2); % create a list of different colours
