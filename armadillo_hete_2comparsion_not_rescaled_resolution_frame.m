function filename_list=armadillo_hete_2comparsion_not_rescaled_resolution_frame

clear all
close all

pname = 'armadillo_hete_not_rescaled';
...
filename_list{1} = '/Users/yujuedwinchen/Dropbox/adobe_internship/hyperbolicFE/sim_data/simulation3D_full_SI_not_rescaled_moving_coarsearmadillo_hete6_stvk_coarsearmadillo/SI_CG_constraint_n_h1_Y800000_P0.45_rho1000_a0_b0_dt0.01/trajectory.mat';


%% second comparision

filename_list{2} = '/Users/yujuedwinchen/Dropbox/adobe_internship/hyperbolicFE/sim_data/simulation3D_full_SI_not_rescaled_moving_armadillo_hete6_stvk_armadillo/SI_CG_constraint_n_h1_Y800000_P0.45_rho1000_a0_b0_dt0.01/trajectory.mat';


%%
colors = [spring(6); bone(6)];
colors = colors(4:6:end,:);
...
%             sky = surf(scal*(skyX-offc),scal*(skyY-offc),scal*(skyZ-offc),'LineStyle','none','FaceColor','flat','FaceLighting','gouraud'); %Make a sphere object.
            ...
            for i_filename = 1:length(trajectories)
                ...
                switch i_filename
                    case 1
                        positions(1:3:end) = positions(1:3:end)+.8;
                        positions(2:3:end) = positions(2:3:end) ;
                        positions(3:3:end) = positions(3:3:end)+1;
                    case 2
                        positions(1:3:end) = positions(1:3:end) -.2;
%                         positions(2:3:end) = positions(2:3:end) ;
                        positions(3:3:end) = positions(3:3:end)+1;
%                     case 3
...
%                 whitebg(cmap)
                ...
                campos([      0.8065  -21.8904    5.5719]);
                camtarget([0.1491   -0.6619    0.9054])
%                 campos([0    2.0   2]);
...
%         print(pname,'-dpdf')      
        cameratoolbar
        print([pname num2str(ti)],'-dpng')
        savefig([pname num2str(ti)])
        cla
    end
