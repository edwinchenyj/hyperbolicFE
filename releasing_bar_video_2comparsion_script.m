function filename_list=releasing_bar_video_2comparsion_script

clear all
close all

video_name = 'CG_releasing_2bar_Y1000_neo-hookean_DAC.avi';
        ...
        if or(mod(ti, draw_rate) == 1, draw_rate == 1)
            ...
            for i_filename = 1:length(trajectories)
                ...
                switch i_filename
                    ...
                    case 2
                        positions(2:3:end) = positions(2:3:end) + 0.12;
                        positions(3:3:end) = positions(3:3:end) + 0.04;
                end
                node = transpose(reshape(positions,3,[]));
