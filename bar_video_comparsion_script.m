function filename_list=bar_video_comparsion_script

clear all
close all

video_name = 'CG_bar_Y1e5_linear.avi';
...
dirname = sprintf('sim_data%csimulation3D_bar_%s_%s', fs,material_type, mesh_shape);
if isDG
    ...
    for ti = 1:tsteps
        ...
        if or(mod(ti, draw_rate) == 1, draw_rate == 1)
            ...
            for i_filename = 1:length(trajectories)
                ...
                switch i_filename
                    case 1
                        positions(2:3:end) = positions(2:3:end) - 0.5;
                    case 2
...
%                 whitebg(cmap)
            ...
            for i_filename = 1:length(trajectories)
                h{i_filename}.EdgeColor = edge_colors{i_filename};
                h{i_filename}.FaceAlpha = 0.5;
                h{i_filename}.EdgeAlpha = 0.3;
            end  
            lh = camlight('headlight');
            lh.Style = 'local';
