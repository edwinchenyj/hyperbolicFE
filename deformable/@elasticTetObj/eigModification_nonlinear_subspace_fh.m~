function [Kh, f] = eigModification_nonlinear_subspace_fh(obj)

if (obj.material_type == 2)
    error('function handle does not work for linear material yet');
...
%     rescaled_K = K + M * V_s * (diag(obj.eig_ratios) - eye(length(obj.eig_ratios))) * (V_s')*K*V_s * (V_s') * M;
    ...
    rescaled_K = @(x) (K*x + M * V_s * (diag(obj.eig_ratios) - eye(length(obj.eig_ratios))) * (diag(low_eig)) * (V_s') * M * x);
