function name = simulation3D_jump_SI_rescaled_scoarsebunny_hete(varargin)
close all
clf
...
% fidn all the points around the same height with the lowest point
bottom_points = find(abs(nodeM(:,3)-nodeM(1,3)) < (0.25/100));
...
indLogical = indLogical(:);
%% find elements close to constraints and make them softer
soft_points = find(abs(nodeM(:,3)-min(nodeM(:,3))) < (0.5/100));
