function simpplot(p,t,expr,bcol,icol,nodes,tris,eda)
...
switch dim
 case 2
  ...
  trimesh(t,p(:,1),p(:,2),0*p(:,1),'facecolor',bcol,'edgecolor','k','edgealpha',eda);
  if nodes==1
    line(p(:,1),p(:,2),'linest','none','marker','.','col',icol,'markers',24,'edgealpha',eda);
  elseif nodes==2
    for ip=1:size(p,1)
 ...
 case 3
  ...
  if size(t,2)==4
    ...
    if nargin>2 & ~isempty(expr)
      ...
      set(h,'facecolor',icol,'edgecolor','k','edgealpha',eda,'facealpha',eda);
      hold on
    end
  else
    tri1=t;
...
%   h=trimesh(tri1,p(:,1),p(:,2),p(:,3),'edgealpha',eda,'facealpha',eda);
  h=trimesh(tri1,p(:,1),p(:,2),p(:,3));
  hold off
%   set(h,'facecolor',bcol,'edgecolor','k','edgealpha',eda,'facealpha',eda);
