%  [w, err, hump] = expv_modified( t, A, v, tol, m )
...
function [w, err, hump, stages, m] = expv_modified_eig( t, A, v, tol, m, ratio )
...
while t_now < t_out
  ...
  while ireject <= mxrej,
     mx = mb + k1;
     % modify the ratios
     [eigv,eigd] = eig(sgn*t_step*H(1:mx,1:mx));
%      F = expm(sgn*t_step*H(1:mx,1:mx));
        for i_ratio = 1:length(ratio)
            eigd(i_ratio,i_ratio) = ratios(i_ratio) * eigd(i_ratio,i_ratio);
        end
        for i_m = 1:m
            eigd(i_m,i_m) = exp(eigd(i_m,i_m));
        end
        F = eigv*eigd/eigv;
     if k1 == 0,
	err_loc = btol; 
