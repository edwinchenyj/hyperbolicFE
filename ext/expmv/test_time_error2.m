...
m_max = [55];
p_max = [8];
i = [1:size(expm_exact2{1},2)]';
j = [1:length(expm_exact2)]';
k = 10.^i;
n = 10.*j;
% load('expm_exact.mat')
...
for n_index = 1:length(n)
    ...
    for stiffness_index= 1:length(k)
        ...
        [X,s(stiffness_index), m(stiffness_index)] = expmv(tvals, A_tilde, b, M);
        ...
        Y(:,1) = expm_exact2{n_index}(:,stiffness_index);
    ...
    fprintf('n = %d\n', n(n_index));
    fprintf('------------------\n')
    fprintf('stiffness k = %8.0f, s = %2.0f, m = %2.0f, err = %8.8e, selection time = %8.8e,  expmv time = %8.8e, backslash time = %8.8e.\n', [k, s, m, err, select_time, expmv_time, backslash_time]')
    fprintf('selection time / expmv time = %8.4e.\n',[select_time./expmv_time]);
    fprintf('expmv time / backslash time = %8.4e.\n',[expmv_time./backslash_time]);
end
