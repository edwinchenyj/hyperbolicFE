function name = simulation3D_free_bar(varargin)
close all
clf
...
indAll = 1:N;
indLogical =true((obj.Dim*N),1);



% if (exist([dirname fs 'eigenmodes.mat'], 'file') ~= 2)
if 1 
    % first set the initial deformation
...
%     firstMode = V(:,4)/2;
    ...
    save([simdir fs 'eigenmodes.mat'], 'obj','V','D'); % storing eigen decomp
else
    load([simdir fs 'eigenmodes.mat']);
    %     load([filename '.mat'], );
end

deformation_mode = zeros(obj.Dim*N,1);
for i_def = 1:3
deformation_mode = deformation_mode + V(:,6 + i_def)/50/i_def;
end
Dx = deformation_mode;

save([simdir fs 'deformation.mat'], 'Dx'); % storing deformation
