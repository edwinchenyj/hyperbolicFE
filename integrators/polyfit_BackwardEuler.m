function u_new = polyfit_BackwardEuler( dt, u, obj, varargin)
% inputs:
%   dt: step size
%    u: current state
%  obj: the elastic obj
...
%     Eforce_end = obj.ElasticForce;
    ...
    if (it > 3 && residual > residual0) || it == MaxIT
        ...
        if nargin > 3
            u_half = polyfit_BackwardEuler(dt/2, u, obj, varargin{1});
        else
            u_half = polyfit_BackwardEuler(dt/2, u, obj);
        end
        ...
        if nargin > 3
            u_new = polyfit_BackwardEuler(dt/2, u_half, obj, varargin{1});
        else
            u_new = polyfit_BackwardEuler(dt/2, u_half, obj);
        end
        break;
    end
