function u_new = polyfit_f_int_ImplicitMid( dt, u, obj, varargin)
% inputs:
%   dt: step size
%    u: current state
%  obj: the elastic obj
...
Eforce_mid = obj.polyfit_force_approx;
...
obj.polyfit_force_approx = obj.polyfit_force_approx - K_mid *  1/2 * dt * (v(indLogical) + v_new(indLogical));
...
%     Eforce_mid = Mass*Minv_K_new*(K_mid\Eforce_mid(indLogical));
    ...
    if (it > 3 && residual > residual0) || it == MaxIT
        ...
        if nargin > 3
            u_half = polyfit_f_int_ImplicitMid(dt/2, u, obj, varargin{1});
        else
            u_half = polyfit_f_int_ImplicitMid(dt/2, u, obj);
        end
        ...
        if nargin > 3
            u_new = polyfit_f_int_ImplicitMid(dt/2, u_half, obj, varargin{1});
        else
            u_new = polyfit_f_int_ImplicitMid(dt/2, u_half, obj);
        end
        break;
    end
