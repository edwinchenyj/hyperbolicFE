function name = simulation3D_3modes_bar(varargin)
close all
clf
...
% 
% 
% vibration = load([simdir fs 'deformation.mat'], 'Dx'); % load eigen decomp
...
dirname = sprintf('sim_data%c%s_%s_%s', fs, [mfilename num2str(polyfit_modes)],material_type, mesh_shape);
...
% [V,D] = eigs(K,M,polyfit_modes,'sm');
% [low_eig, permutation_indices] = sort(diag(D));
% save('h3_low_res_K_M_bar_fixed','K','M');

% obj.polyfit_p = polyfit([0;low_eig(1:polyfit_modes)],[0;high_res_low_eig(1:polyfit_modes)],polyfit_modes);
% obj.polyfit_p(end) = 0;
% ratio = high_res_low_eig(1)/low_eig(1);
% x1 = linspace(0,1e4);
% y1 = polyval(obj.polyfit_p,x1);
% plot(x1,y1);
...
if (exist([simdir fs 'trajectory.mat'], 'file') ~= 2)
    ...
    for ti = 1:tsteps
        ...
        switch solver
            case 'IM'
                u = eig_ImplicitMid(dt, u, obj,~indLogical);
            case 'SI'
                u = eig_SemiImplicit(dt, u, obj,~indLogical);
            case 'SIModified'
            ...
            case 'RK4'
                u = polyfit_RK4(dt, u, obj, ~indLogical);
        end
