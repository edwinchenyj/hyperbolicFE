...
parameter_lists = {...
    ...
    'dt',[1/100],...
    'T',[1],...
    'mesh_shape',{'coarsebunny','vcoarsebunny'},...
    'simulation_type',{'CG'},...
    'solver',{'IM'},...
    'Y',8e2,...
    'P',0.45,...
...
for ti = 1:tsteps
    ...
    if or(mod(ti, draw_rate) == 1, draw_rate == 1)
        ...
        for i_filename = 1:length(trajectories)
            ...
            switch i_filename
                case 1
%                     positions(1:3:end) = positions(1:3:end) - 0.0015;
                    %                         positions(2:3:end) = positions(2:3:end) - 0.3;
%                     positions(3:3:end) = positions(3:3:end) + 0.0024;
                case 2
                    positions(1:3:end) = positions(1:3:end) + 0.002;
