function name = simulation3D_reduced_ERE_rescaled_shake_fast_vcoarsebunny(varargin)
close all
clf
...
if exist([meshname '_' material_type num2str(Y) '_Eobj.mat'], 'file') ~= 2
...
high_res_low_eig = low_eig;
    save([meshname '_' material_type num2str(Y) '_Eobj.mat'], 'high_res_low_eig');

else
    load([meshname '_' material_type num2str(Y) '_Eobj.mat'], 'high_res_low_eig');
...
%         if (ti < 51) || (ti > 150)
        if mod(ti,40) < 20
            ...
            for bottom_i = bottom_points'
                temp = positions((3*(bottom_i-1)+1):3*bottom_i) + [1 0 0]' * 0.005 * dt;
