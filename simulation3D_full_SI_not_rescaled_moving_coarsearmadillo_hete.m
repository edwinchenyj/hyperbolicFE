function name = simulation3D_full_SI_not_rescaled_moving_coarsearmadillo_hete(varargin)
close all
clf
...
Y = Y * ones(size(elem,1),1);
P = P * ones(size(elem,1),1);
rho = rho * ones(size(elem,1),1);
...
switch material_type
    case 'linear'
        obj.SetMaterial( Y, P, rho, 2 * ones(size(elem,1),1) , a, b); % set the tri to linear
        %     obj.SetMaterial( Y, P, rho, 1:size(elem,1), 1); % set the tri to neo-hookean
    case 'neo-hookean'
        obj.SetMaterial( Y, P, rho, 1 * ones(size(elem,1),1), a, b); % set the tri to neo-hookean
    case 'stvk'
        obj.SetMaterial( Y, P, rho, 3 * ones(size(elem,1),1), a, b); % set the tri to stvk
end
    ...
    for ti = 1:tsteps
        ...
        if(draw)
            if or(mod(ti, draw_rate) == 1, draw_rate == 1)
                ...
                fig.Children.Clipping = 'off';
                fig.Children.Projection = 'perspective';
                campos([-5.0221   -6.5465    4.5846]);
                camtarget([0.0001   -0.0014   -0.1785])
                camva(10.7619);
                scal = 2000;
                offc = 0.1;
                hold on
