function filename_list=bar_video_2comparsion_DAC_script

clear all
close all

video_name = 'CG_bar_Y4000_neo-hookean_DAC.avi';
...
filename_list{1} = 'sim_data\simulation3D_motions_bar_DAC_neo-hookean_small_bar\SI_CG_constraint_n_h0.3_Y4000_P0.3_rho1000_a0_b0_dt0.01_def-scln\trajectory.mat';


%% second comparison

filename_list{2} = 'sim_data\simulation3D_motions_bar_DAC_neo-hookean_small_bar\SI_CG_constraint_n_h0.2_Y4000_P0.3_rho1000_a0_b0_dt0.01_def-scln\trajectory.mat';
...
%                 ph = phlist(i_filename);
                ...
                %                 lh.Position(3) = 3;
                cmap = [0.8 0.9 1];
                h{i_filename}.FaceVertexCData = repmat(edge_colors{i_filename+2},Ns(i_filename),1);
                %                 colormap(autumn);
                
                shading faceted
                h{i_filename}.EdgeColor = edge_colors{i_filename+2};
...
%                 whitebg(cmap)
                ...
                campos([-2.1606   -0.2154    1.9364]);
                camtarget([0.1378   -0.2154    0.1897])
%                 campos([0    2.0   2]);
...
%                 add_shadow()
            ...
            for i_filename = 1:length(trajectories)
                t = h{i_filename};
                t.EdgeColor = edge_colors{i_filename+2};
                t.FaceAlpha = 0.5;
                t.EdgeAlpha = 0.6;
